...
public static class ViewModelLocationProvider
{
    ...
    /// It first looks to see if there is a mapping registered for that view, if not it will fall back to the convention based approach.
    /// </summary>
    ...
    public static void AutoBindingViewModelChanged(object view, Action<object, object> setDataContextCallback)
    {
        // Try mappings first
        ...
        if (viewModel == null)
        {
            ...
            var viewModelType = (GetViewModelTypeForView(view.GetType())
                                 ?? _defaultViewToViewModelTypeResolver(view))
                                ?? _defaultViewTypeToViewModelTypeResolver(view.GetType());
    ...
    private static Type? GetViewModelTypeForView(Type view)
    {
        ...
        return _typeFactories.GetValueOrDefault(viewKey);
    }
